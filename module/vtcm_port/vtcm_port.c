        ...
        if (ret > 0) {
			...
			switch(output_data->tag)
			{
				...
				//		type=DTYPE_VTCM_IN_AUTH2;
					...
					//Get the entire command template
                        		...
                        		else 
                        		{
                                		...
                                		ret = blob_2_struct(messagebuf, Extend_input, command_template);
				...
				{	
					...
					output_cmd=(struct vtcm_manage_cmd_head *)malloc(sizeof(*output_cmd));
                        		ret = blob_2_struct(messagebuf, output_cmd,command_template) ;
					offset=ret;
                        		output_data = (struct vtcm_external_input_command *)malloc(extend_size) ;
                        		ret = blob_2_struct(messagebuf+offset, output_data,extend_template) ;
					type=DTYPE_VTCM_IN;
					subtype=output_data->ordinal;
					...
					//Get the entire command template
                        		...
                        		{
                                		void * Extend_input = malloc(struct_size(command_template));
                                		ret = blob_2_struct(messagebuf+offset, Extend_input, command_template);
                                		void * send_msg = message_create(type,subtype,NULL);
                                		if(send_msg == NULL)
                                			return -EINVAL;
                                		message_add_record(send_msg, Extend_input);
	    ...
	    *(int *)(cmd_buf+2)=htonl(blob_size);
            if(offset>0)
	    {
		    *(int *)(sendbuf+2)=htonl(offset+blob_size);
            ...
            send_conn = getConnectorByIp(hub, "127.0.0.1");
            if (send_conn != NULL) {
                printf("write to conn peeraddr %s\n", send_conn->conn_peeraddr);
                int len = send_conn->conn_ops->write(send_conn, sendbuf, cmdlen);
                if (len == cmdlen)
