...
int proc_vtcm_SM2Decrypt(void *sub_proc, void* recv_msg)
{
    ...
    //input process
    struct tcm_in_Sm2Decrypt *vtcm_in;
    struct tcm_out_Sm2Decrypt *vtcm_out;
    void * vtcm_template;
    int   returnCode;
    ...
    */
    ...
    if(ret == TCM_SUCCESS)
    {
        ret = vtcm_Compute_AuthCode_Sm2Decrypt(ret,
                                               vtcm_in->ordinal,
                                               vtcm_out->DecryptedDataSize,
...
sm2decrypt_out:
    ...
    vtcm_out->paramSize = responseSize;
    void *send_msg = message_create(DTYPE_VTCM_OUT ,SUBTYPE_SM2DECRYPT_OUT ,recv_msg);
    if(send_msg == NULL)
    {
        printf("send_msg == NULL\n");
        return -EINVAL;      
...
int proc_vtcm_Sm4Encrypt(void * sub_proc,void * recv_msg)
{
    ...
    if(ret == TCM_SUCCESS) {
        vtcm_AuthData_Checkin_Sm4(tcm_input->ordinal,
                                tcm_input->CBCusedIV, 
                                tcm_input->EncryptDataSize, 
                                tcm_input->EncryptData, 
                                auth_session_data, 
                                tcm_input->EncryptAuthVerfication);
    }
    ...
    if(ret == TCM_SUCCESS) {
	/*
        vtcm_AuthData_Checkout_Sm4(tcm_output->returnCode, 
                                   tcm_input->ordinal, 
                                   tcm_output->EncryptedDataSize, 
                                   tcm_output->EncryptedData,
                                   auth_session_data, 
...
int proc_vtcm_Sm4Decrypt(void * sub_proc,void * recv_msg)
{
    ...
    if(ret == TCM_SUCCESS) {
        vtcm_AuthData_Checkin_Sm4(tcm_input->ordinal,
                                tcm_input->CBCusedIV, 
                                tcm_input->DecryptDataSize, 
                                tcm_input->DecryptData, 
                                auth_session_data, 
                                tcm_input->DecryptAuthVerfication);
    }
...
int proc_vtcm_Sign(void * sub_proc,void * recv_msg)
{
    ...
    if(ret == TCM_SUCCESS) {
        vtcm_AuthData_Check_Sign(tcm_input->ordinal,
                                 tcm_input->areaToSignSize,
                                 tcm_input->areaToSign,
                                 auth_session_data,
                                 tcm_input->privAuth);
    }
    ...
    if(ret == TCM_SUCCESS) {
        vtcm_AuthData_Check_Signout(tcm_output->returnCode,
                                    tcm_input->ordinal,
                                    tcm_output->sigSize,
                                    tcm_output->sig,
                                    auth_session_data,
...
int proc_vtcm_Seal(void *sub_proc, void *recv_msg)
{
    ...
    if(ret == TCM_SUCCESS)
    {
        ret = vtcm_Check_AuthCode_Seal(vtcm_input->ordinal,
                                       vtcm_input->encAuth,
                                       vtcm_input->pcrInfoSize,
                                       vtcm_input->pcrInfo,
                                       vtcm_input->InDataSize,
                                       vtcm_input->InData,
                                       authSession,
                                       vtcm_input->authCode);
    }
    ...
    if(ret == TCM_SUCCESS)
    {    
        ret = vtcm_Compute_AuthCode_Seal(ret,
                                         vtcm_input->ordinal,
                                         &(vtcm_output->sealedData),
                                         authSession,
...
int proc_vtcm_UnSeal(void *sub_proc, void *recv_msg)
{
    ...
    if(ret == TCM_SUCCESS)
    {
        ret = vtcm_Check_AuthCode_UnSeal(vtcm_input->ordinal,
                                         &(vtcm_input->encAuth),
                                         authSession,
                                         vtcm_input->authCode);
    }
