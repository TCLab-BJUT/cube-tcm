...
int proc_vtcmutils_SM2Decrypt(void * sub_proc, void * para){
	...
	(input_para->param_num%2==1))
 {
  ...
  {
  	// Check authdata
  	vtcm_template=memdb_get_template(DTYPE_VTCM_OUT_AUTH1,SUBTYPE_SM2DECRYPT_OUT);
  	if(vtcm_template==NULL)
    		return -EINVAL;
  	ret=blob_2_struct(Buf,vtcm_output,vtcm_template);
  	if(ret<0)
...
int proc_vtcmutils_LoadKey(void * sub_proc, void * para){
  ...
  // Check authdata
  vtcm_template = memdb_get_template(DTYPE_VTCM_OUT_AUTH1,SUBTYPE_LOADKEY_OUT);
  if(vtcm_template==NULL)
    return -EINVAL;
  ret=blob_2_struct(Buf,vtcm_output,vtcm_template);
  if(ret<0)
